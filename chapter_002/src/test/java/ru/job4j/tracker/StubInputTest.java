...
public class StubInputTest {
    ...
    public void whenUserUpdateItemThenTrackerUpdateSelectedItem() {
        ...
        Output output = new ConsoleOutput();
        new StartUI(input, output, tracker).init();
        input = new StubInput(new String[]{"2", tracker.findAll().get(0).getId(), "test3", "desc3", "6"});
        new StartUI(input, output, tracker).init();
        assertThat(tracker.findAll().get(0).getName(), is("test3"));
    }
    /**
    ...
    public void whenUserFindItemByIdThenTrackerSelectThatItem() {
        ...
        Output output = new ConsoleOutput();
        new StartUI(input, output, tracker).init();
        assertThat(tracker.findAll().get(0), is(tracker.findById(tracker.findAll().get(0).getId())));
    }
    /**
    ...
    public void whenUserFindItemByNameThenTrackerSelectThatItems() {
        ...
        Output output = new ConsoleOutput();
        new StartUI(input, output, tracker).init();
        assertThat(new Item[]{tracker.findAll().get(0)}, is(tracker.findByName(tracker.findAll().get(0).getName())));
    }
}
